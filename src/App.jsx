  ...
  return (
    <div className="container">
      ...
      <div className="card">
        ...
        <div className={`instructions ${showHelp ? 'show' : ''}`}>
          <div className="info-box">
            <p><b>Boss Drop Tracker</b><br />Catatan item drop dari boss milik <a href="https://www.rucoyonline.com/characters/Xzydanzz" target="_blank" rel="noreferrer" style={{color: '#1bd18a'}}>Xzydanzz</a>.</p>
          </div>
        </div>
        ...
        <button onClick={() => window.location.reload()}><i className="fa-solid fa-refresh"></i> Refresh Page</button>
      </div>

      <div className="card">
        <div className="stats">
          <div className="stats-item"><span>Total Drops:</span><span className="stats-value">{stats.total}</span></div>
          <div className="stats-item"><span>Ultra Rare:</span><span className="stats-value">{stats.ultra}</span></div>
          <div className="stats-item"><span>Blue Rare:</span><span className="stats-value">{stats.blue}</span></div>
        </div>
      </div>
            ...
            <div key={index} className={`drop-item ${item.type} ${expandedId === index ? 'expanded' : ''}`} onClick={() => setExpandedId(expandedId === index ? null : index)}>
              <div className="drop-header">
                <div className={`drop-icon ${item.noIconBg ? 'no-bg' : ''}`}>
                  <img src={item.img} alt={item.nama} style={{width: '24px', height: '24px'}} />
                </div>
              ...
              <div className="drop-details">
                <div className="drop-details-content">
                  <div className="stats-item"><span>Boss:</span><span>{item.boss}</span></div>
                  <div className="stats-item"><span>Date:</span><span>{item.tanggal}</span></div>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="footer">
        © 2026 <a href="https://www.rucoyonline.com/" target="_blank" rel="noreferrer" style={{color: '#4caf50'}}>Rucoy Online</a>
      </div>
    </div>
  )
}
